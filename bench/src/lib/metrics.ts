import { MetricKey } from '../enums.js';

// ── Metric Metadata ──────────────────────────────────────────────────────────

export interface MetricInfo {
    displayName: string;
    unit: string;
    category: 'size' | 'time' | 'throughput' | 'memory';
}

export const METRICS: Record<MetricKey, MetricInfo> = {
    [MetricKey.CompileTime]: { displayName: 'Compile Time', unit: 'ms', category: 'time' },
    [MetricKey.DiskSizeNative]: { displayName: 'Disk Size (WASM)', unit: 'bytes', category: 'size' },
    [MetricKey.DiskSizeAssemblies]: { displayName: 'Disk Size (DLLs)', unit: 'bytes', category: 'size' },
    [MetricKey.DownloadSizeCold]: { displayName: 'Download Size Cold', unit: 'bytes', category: 'size' },
    [MetricKey.DownloadSizeWarm]: { displayName: 'Download Size Warm', unit: 'bytes', category: 'size' },
    [MetricKey.ServerRequestsCold]: { displayName: 'Server Requests (Cold)', unit: 'count', category: 'size' },
    [MetricKey.ServerRequestsWarm]: { displayName: 'Server Requests (Warm)', unit: 'count', category: 'size' },
    [MetricKey.TimeToReachManagedWarm]: { displayName: 'Time to Managed (Warm)', unit: 'ms', category: 'time' },
    [MetricKey.TimeToReachManagedCold]: { displayName: 'Time to Managed (Cold)', unit: 'ms', category: 'time' },
    [MetricKey.TimeToCreateDotnetWarm]: { displayName: 'Time to Create Dotnet (Warm)', unit: 'ms', category: 'time' },
    [MetricKey.TimeToCreateDotnetCold]: { displayName: 'Time to Create Dotnet (Cold)', unit: 'ms', category: 'time' },
    [MetricKey.TimeToExitWarm]: { displayName: 'Time to Exit (Warm)', unit: 'ms', category: 'time' },
    [MetricKey.TimeToExitCold]: { displayName: 'Time to Exit (Cold)', unit: 'ms', category: 'time' },
    [MetricKey.WasmMemorySize]: { displayName: 'WASM Memory Size', unit: 'bytes', category: 'memory' },
    [MetricKey.MemoryPeak]: { displayName: 'Peak JS Heap', unit: 'bytes', category: 'memory' },
    [MetricKey.PizzaWalkthrough]: { displayName: 'Pizza Walkthrough', unit: 'ms', category: 'time' },
    [MetricKey.HavitWalkthrough]: { displayName: 'Havit Walkthrough', unit: 'ms', category: 'time' },
    [MetricKey.MudWalkthrough]: { displayName: 'Mud Walkthrough', unit: 'ms', category: 'time' },
    [MetricKey.IgniteUIWalkthrough]: { displayName: 'IgniteUI Walkthrough', unit: 'ms', category: 'time' },
    [MetricKey.UnoWalkthrough]: { displayName: 'Uno Walkthrough', unit: 'ms', category: 'time' },
    [MetricKey.SemiWalkthrough]: { displayName: 'Semi Avalonia Walkthrough', unit: 'ms', category: 'time' },
    [MetricKey.BlazorJsToCsNumber]: { displayName: 'JS→CS Number/sec', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorJsToCsString]: { displayName: 'JS→CS String/sec', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorJsToCsJson]: { displayName: 'JS→CS JSON/sec', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorCsToJsNumber]: { displayName: 'CS→JS Number/sec', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorCsToJsString]: { displayName: 'CS→JS String/sec', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorCsToJsJson]: { displayName: 'CS→JS JSON/sec', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorCounterHeavyWasm]: { displayName: 'Counter Heavy (WASM)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorCounterHeavyServer]: { displayName: 'Counter Heavy (Server)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorParamsCountWasm]: { displayName: 'Params Count (WASM)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorParamsCountServer]: { displayName: 'Params Count (Server)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorTooManyComponentsWasm]: { displayName: 'Many Components (WASM)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorTooManyComponentsServer]: { displayName: 'Many Components (Server)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorParamsCountSsr]: { displayName: 'Params Count (SSR)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorTooManyComponentsSsr]: { displayName: 'Many Components (SSR)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorParamsCountSsrStress]: { displayName: 'Params Count (SSR ×100)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorTooManyComponentsSsrStress]: { displayName: 'Many Components (SSR ×100)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorParamsCountHtmlRenderer]: { displayName: 'Params Count (HtmlRenderer)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorTooManyComponentsHtmlRenderer]: { displayName: 'Many Components (HtmlRenderer)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorParamsCountHtmlRendererStress]: { displayName: 'Params Count (HtmlRenderer ×10)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorTooManyComponentsHtmlRendererStress]: { displayName: 'Many Components (HtmlRenderer ×10)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorParamsCountServerStress]: { displayName: 'Params Count (Server ×25)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.BlazorTooManyComponentsServerStress]: { displayName: 'Many Components (Server ×25)', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.JsInteropOps]: { displayName: 'JS Interop', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.JsonParseOps]: { displayName: 'JSON Parse', unit: 'ops/sec', category: 'throughput' },
    [MetricKey.ExceptionOps]: { displayName: 'Exception Handling', unit: 'ops/sec', category: 'throughput' },
};

export const EXTERNAL_METRICS: MetricKey[] = [
    MetricKey.CompileTime,
    MetricKey.DiskSizeNative,
    MetricKey.DiskSizeAssemblies,
    MetricKey.DownloadSizeCold,
    MetricKey.DownloadSizeWarm,
    MetricKey.ServerRequestsCold,
    MetricKey.ServerRequestsWarm,
    MetricKey.TimeToReachManagedWarm,
    MetricKey.TimeToReachManagedCold,
    MetricKey.TimeToCreateDotnetWarm,
    MetricKey.TimeToCreateDotnetCold,
    MetricKey.TimeToExitWarm,
    MetricKey.TimeToExitCold,
    MetricKey.WasmMemorySize,
    MetricKey.MemoryPeak,
    MetricKey.PizzaWalkthrough,
    MetricKey.HavitWalkthrough,
    MetricKey.MudWalkthrough,
    MetricKey.IgniteUIWalkthrough,
    MetricKey.UnoWalkthrough,
    MetricKey.SemiWalkthrough,
    MetricKey.BlazorJsToCsNumber,
    MetricKey.BlazorJsToCsString,
    MetricKey.BlazorJsToCsJson,
    MetricKey.BlazorCsToJsNumber,
    MetricKey.BlazorCsToJsString,
    MetricKey.BlazorCsToJsJson,
    MetricKey.BlazorCounterHeavyWasm,
    MetricKey.BlazorCounterHeavyServer,
];

export const INTERNAL_METRICS: MetricKey[] = [
    MetricKey.CompileTime,
    MetricKey.MemoryPeak,
    MetricKey.TimeToCreateDotnetCold,
    MetricKey.TimeToExitCold,
    MetricKey.WasmMemorySize,
    MetricKey.JsInteropOps,
    MetricKey.JsonParseOps,
    MetricKey.ExceptionOps,
];
