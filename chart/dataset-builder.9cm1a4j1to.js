...
import { ENGINE_COLORS, PRESET_DASH, RUNTIME_MARKER, PROFILE_LINE_WIDTH, BUILD_METRICS, WALKTHROUGH_METRICS, assert, } from './constants.js';
...
export async function buildReleasePoints(dataBaseUrl, app, metric, releaseBuckets, tickLayout) {
...
export async function buildWeekDatasets(dataBaseUrl, app, metric, weekBuckets, tickLayout) {
    const datasets = [];
    ...
    for (let wi = 0; wi < filteredBuckets.length; wi++) {
        ...
        for (const [rowKey, values] of Object.entries(metricData)) {
            ...
            if (points.length === 0)
                continue;
            const existingIdx = datasets.findIndex(d => d.rowKey === rowKey && d.zone === 'active');
            if (existingIdx >= 0) {
