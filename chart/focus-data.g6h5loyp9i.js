...
export function focusMetrics(app) {
    return [
...
export function isSdk12(column) {
    return column.major === 12 && column.channel === '12.0' && column.sdkVersion.startsWith('12.');
}
export function isEligible(column) {
    return isSdk12(column) && column.isRuntimeCustomBuild !== true && column.isAspnetCoreCustomBuild !== true;
}
export function isPositive(value) {
    return value !== null && Number.isFinite(value) && value > 0;
}
export function percentVsMono(coreclr, mono) {
    if (!isPositive(coreclr) || !isPositive(mono))
        return null;
    const percent = 100 * (coreclr / mono - 1);
    return Number.isFinite(percent) ? percent : null;
}
export function comparisonLabel(value, unit) {
    if (!Number.isFinite(value))
        throw new RangeError('A comparison must be finite.');
    const rounded = Math.abs(value).toFixed(1);
    if (Number(rounded) === 0)
        return { value, magnitude: '0.0%', verdict: 'same', tone: 'neutral' };
    return {
...
export function formatFocusValue(value, metric) {
...
export function historyBounds(range, now) {
    if (!Number.isFinite(now.getTime()))
        throw new RangeError('The current date is invalid.');
...
export function orderObservations(a, b) {
    return a.day.localeCompare(b.day)
        || a.sdkVersion.localeCompare(b.sdkVersion, 'en', { numeric: true })
        || a.id.localeCompare(b.id);
}
export function rollingWindows(values, observations, size = FOCUS_WINDOW_SIZE) {
    if (!Number.isInteger(size) || size < 1)
        throw new RangeError('Window size must be a positive integer.');
    if (values.length !== observations.length)
        throw new RangeError('Every value needs an observation.');
    let window = [];
...
function pairIssue(coreclr, mono, percent) {
    if (mono !== null && !isPositive(mono))
        return 'Invalid Mono baseline: a finite positive value is required.';
    if (coreclr !== null && !isPositive(coreclr))
        return 'Invalid CoreCLR measurement: a finite positive value is required.';
    if (mono === null && coreclr === null)
        return 'Neither runtime has a measurement in this cohort.';
    if (mono === null)
        return 'Mono measurement missing; no baseline was substituted.';
    if (coreclr === null)
        return 'CoreCLR measurement missing; no runtime was substituted.';
    return percent === null ? 'The comparison exceeds the finite numeric range.' : null;
}
export function buildFocusReport(publication, app, range, now = new Date(), selection = DEFAULT_FOCUS_SELECTION) {
    const flavor = resolveFocusSelection(selection);
    if (!publication.index.apps.includes(app))
        throw new FocusPublicationError(`Application '${app}' is not in the published index.`);
    ...
    for (const bucket of publication.buckets) {
        bucket.header.columns.forEach((column, columnIndex) => {
            ...
            if (seen.has(id))
                throw new FocusPublicationError(`Duplicate SDK/variant identity in ${bucket.path}: ${column.sdkVersion}. Retry the publication.`);
            seen.add(id);
    ...
    const metrics = focusMetrics(app).map(definition => {
        ...
        const allPoints = observations.map(observation => {
            ...
            if (available && !rows)
                throw new FocusPublicationError(`Advertised ${app}/${definition.key} was not loaded from ${bucket.path}.`);
        ...
        return {
            ...
            description: `${definition.description} ${flavor.coreclrLabel} vs ${flavor.monoLabel}; ${profile}/Chromium. ${profile === 'mobile' ? 'Mobile emulation: 3x CPU slowdown, 20 Mbps download, 5 Mbps upload, 70 ms latency.'
