...
export const DEFAULT_FOCUS_SELECTION = {
    flavor: 'r2r-release',
    startupProfile: 'mobile',
};
export const DEFAULT_FOCUS_GRAPH_VISIBILITY = {
    percentage: false,
    measurements: true,
};
export const DEFAULT_FOCUS_AVERAGED = false;
export function resolveFocusSelection(selection) {
    const flavor = FOCUS_FLAVORS.find(option => option.id === selection.flavor);
    if (!flavor)
        throw new RangeError(`Unsupported comparison flavor: ${selection.flavor}`);
...
export function focusRowKeys(flavor, profile) {
    return {
